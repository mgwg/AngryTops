...
def plot_observables(obs):
    # Load the histograms
    hname_true = "Wb_%s_true" % (obs)
    hame_fitted = "Wb_%s_fitted" % (obs)
    ...
    # t graphs
    t_name_true = "t_%s_true" % (obs)
    t_name_fitted = "t_%s_fitted" % (obs)
    t_true = infile_plot.Get(t_name_true)
    t_fitted = infile_plot.Get(t_name_fitted)
    print(t_name_true, t_name_fitted)
    ...
    # t graph
    SetTH1FStyle( t_true, color=46, markersize=0, markerstyle=20, linewidth=3 ) # red
    SetTH1FStyle( t_fitted, color=38, markersize=0, markerstyle=20, linewidth=3 ) # blue
    ...
    h_true.SetMinimum( 0. )
    # t graph
    t_fitted.SetMaximum( hmax )
    t_true.SetMaximum( hmax )
    t_fitted.SetMinimum( 0. )
    t_true.SetMinimum( 0. )
    ...
    leg.AddEntry( h_true, "MG5+Py8 W + b", "f" )
    leg.AddEntry( h_fitted, "Predicted W + b", "f" )
    leg.AddEntry( t_true, "MG5+Py8 t", "f" )
    leg.AddEntry( t_fitted, "Predicted t", "f" )
    ...
    c.SaveAs("{0}/E_fit/{1}.png".format(training_dir, obs))
    pad0.Close()
    pad1.Close()
    c.Close()
