...
hists['corr_max_jet_v_had_W_pt'] = TH2F( "corr_max_jet_v_had_W_pt",   ";Leading Jet p_{T} (GeV);Hadronic W p_{T} (GeV)", 50, 0, 400 , 50, 0, 400 )
...
for i in range(n_events):
    ...
    # hists['max_jet_pt'].Fill(jets_Pt[0])
    # hists['jet_had_W_pt_diff'].Fill( jets_Pt[0]-W_had_true.Pt() )
    hists['had_W_pt'].Fill(W_had_true.Pt())
    hists['corr_max_jet_v_had_W_pt'].Fill( jets_Pt[0], W_had_true.Pt() )
...
def plot_hist(h, obs, caption):
    # h = hists['jet_had_W_pt_diff']

    # Axis titles
    xtitle = h.GetXaxis().GetTitle()
    ytitle = h.GetYaxis().SetTitle("A.U.")

    # Set Style
    SetTH1FStyle( h,  color=kGray+2, fillstyle=1001, fillcolor=kGray, linewidth=3, markersize=0 )
    ...
    # yrange = [0.4, 1.6]
    # frame, tot_unc, ratio = DrawRatio(h, h_quark, xtitle, yrange)
    gPad.RedrawAxis()
...
def plot_observables(h_jet, h_quark, obs, caption):
    # h_jet = hists['max_jet_pt']
    # h_quark = hists['had_W_pt']
...
# plot_observables('jet_v_W_pt', 'Leading Jet p_{T} vs Hadronic W p_{T}')
# plot_hist('jet_W_pt_diff', 'Leading Jet p_{T} - Hadronic W p_{T}')
plot_correlations('corr_jet_v_W_pt', 'Leading Jet p_{T} vs Hadronic W p_{T}')


for i in range(5):
    plot_observables(hists['max_jet_{}_pt'.format(i)], hists['had_W_pt'], 'jet_{}_v_W_pt'.format(i), str(i+1)+' Leading Jet p_{T} vs Hadronic W p_{T}')
    plot_hist(hists['jet_{}_had_W_pt_diff'.format(i)], 'jet_{}_W_pt_diff'.format(i), str(i+1)+' Leading Jet p_{T} - Hadronic W p_{T}')
    # hists['max_{}_jet_pt'.format(i)].Fill(jets_Pt[i])
    # hists['jet_{}_had_W_pt_diff'.format(i)].Fill( jets_Pt[i]-W_had_true.Pt() )