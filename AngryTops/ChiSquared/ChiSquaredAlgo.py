...
def MakeNeutrino(lep_Px, lep_Py, lep_Pz, met, met_phi):
    ...
    lep = TLorentzVector().SetPxPyPzE(lep_Px, lep_Py, lep_Pz, 0)
    l_px = lep.Px()
    l_py = lep.Py()
    l_pz = lep.Pz()
    l_m  = lep.M()
    ...
    pT_vl = v_px*l_px + v_py*l_py // pT( v * l )
...
def ChiSquared(jets, mu, lep):
    ...
    chi2 =  (W_had.M() - mW)**2 / sigmaW**2 +
            (W_lep.M() - mW)**2 / sigmaW**2 +
            (t_had.M() - mT)**2 / sigmaT**2 +
            (t_lep.M() - mT)**2 / sigmaT**2

    return chi2, [jets[0], jets[1], W_had, W_lep, t_had, t_lep]


def reconstruct(jets_array, mu, lep):
...
def Predict(lep_arr, jet_arr):
    ...
    """
    lep = TLorentzVector().SetPxPyPzM(lep_arr[0], lep_arr[1], lep_arr[2], 0)
    ...
    for i in range(len(jet_arr)):
        j = TLorentzVector().SetPxPyPzE(jet_arr[i:0], jet_arr[i,1],
                                        jet_arr[i,2], jet_arr[i,3])
        jets.append(j)
...
if __name__=="__main__":
    ...
    jets = jets.reshape(-1, 30)
    jets = jets_scalar.inverse_transform(jets)
    jets = jets.reshape(-1, 6, 5)

    # Predict each event
    chi2_pred = np.zeros(jets.shape[0], 6, 3)
    for i in range(len(jets.shape[0])):
